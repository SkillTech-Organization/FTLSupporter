...
namespace Map.LongProcess
{
   ...
   public class RouteVisDataProcess_DEPRECATED : BaseLongProcess
    {
        private SQLServerAccess m_DB = null;                 //A multithread miatt saját adatelérés kell
        private bllRouteVis_DEPRECATED m_bllRouteVis;
        ...
        public RouteVisDataProcess_DEPRECATED()
            : base(new BaseSilngleProgressDialog(0, RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots.Count - 1, PMapMessages.M_ROUTVIS_LOADDATA_DEPRECATED, false), PMapIniParams.Instance.InitRouteDataProcess)
        {
            ...
            m_bllRouteVis = new bllRouteVis_DEPRECATED(m_DB);
        ...
        protected override void DoWork()
        {
            ...
            List<int> nodes = RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots.Select(i => i.Depot.NOD_ID).ToList();
            ...
            boTruck Truck = RouteVisCommonVars_DEPRECATED.Instance.Truck;
            ...
            double dTollMultiplier = bllPlanEdit.GetTollMultiplier(RouteVisCommonVars_DEPRECATED.Instance.CalcTRK_ETOLLCAT, Truck.TRK_ENGINEEURO);
            RouteVisCommonVars_DEPRECATED.CRouteVis RouteVisS = new RouteVisCommonVars_DEPRECATED.CRouteVis(RouteVisCommonVars_DEPRECATED.TY_SHORTEST, PMapMessages.M_PATH_SHORTEST);
            RouteVisCommonVars_DEPRECATED.CRouteVis RouteVisF = new RouteVisCommonVars_DEPRECATED.CRouteVis(RouteVisCommonVars_DEPRECATED.TY_FASTEST, PMapMessages.M_PATH_FASTEST);

            RouteVisCommonVars_DEPRECATED.Instance.lstDetails.Clear();

            for (int i = 0; i < RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots.Count - 1; i++)
            {
                if (!RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i].ValidRoute)
                {
                    ProcessForm.NextStep();
                    ProcessForm.SetInfoText(RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i].Depot.DEP_NAME + " --> " + RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i + 1].Depot.DEP_NAME);

                    //Legrövidebb út
                    boRoute routeS = provider.GetRoute(RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i].Depot.NOD_ID, RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i + 1].Depot.NOD_ID, routePar,
                    ...
                    if (routeS != null)
                    {
                        genData(RouteVisS, routeS,
                            RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i].Depot.NOD_ID,
                            RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i + 1].Depot.NOD_ID,
                            Truck, dTollMultiplier, RouteVisCommonVars_DEPRECATED.Instance.GetRouteWithTruckSpeeds,
                            RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i].RouteSectionType, ref lastETLCODE_S);
                    }

                    //Leggyorsabb út
                    boRoute routeF = provider.GetRoute(RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i].Depot.NOD_ID, RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i + 1].Depot.NOD_ID, routePar,
                                    NeighborsFull[routePar.Hash], 
                                    PMapIniParams.Instance.CutMapForRouting ? NeighborsCut[routePar.Hash] : null,
                    ...
                    if (routeF != null)
                    {
                        genData(RouteVisF, routeF,
                            ...
                            RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i + 1].Depot.NOD_ID,
                            Truck, dTollMultiplier, RouteVisCommonVars_DEPRECATED.Instance.GetRouteWithTruckSpeeds,
                             RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i].RouteSectionType, ref lastETLCODE_F);
                    }
                ...
                RouteVisCommonVars_DEPRECATED.Instance.lstRouteDepots[i].ValidRoute = false;
                
                RouteVisCommonVars_DEPRECATED.Instance.lstDetails.Clear();

//                if (i < RouteVisCommonVars.Instance.lstDetails.Count - 1)
                {
                    RouteVisCommonVars_DEPRECATED.Instance.lstDetails.Add(RouteVisS);
                    RouteVisCommonVars_DEPRECATED.Instance.lstDetails.Add(RouteVisF);
                }
                //else
                {
//                    RouteVisCommonVars.Instance.lstDetails[i] = RouteVisS;
        ...
        private void genData(RouteVisCommonVars_DEPRECATED.CRouteVis p_RouteVis, boRoute p_route, int p_NOD_ID_FROM, int p_NOD_ID_TO, boTruck p_Truck, double p_TollMultiplier, bool p_CalcWithTruckSpeeds, boXRouteSection_DEPRECATED.ERouteSectionType p_RouteSectionType, ref string p_lastETLCODE)
        {
            ...
            /*******/
            if (p_RouteSectionType == boXRouteSection_DEPRECATED.ERouteSectionType.Empty)
            {
            ...
            /*******/
            if (p_RouteSectionType == boXRouteSection_DEPRECATED.ERouteSectionType.Loaded)
            {
            ...
            double LastSpeed = -1;
            string LastETLCODE = "";
            RouteVisCommonVars_DEPRECATED.CRouteVisDetails detail = null;
            foreach (boEdge edge in p_route.Edges)
            {
                ...
                {
                    detail = new RouteVisCommonVars_DEPRECATED.CRouteVisDetails(p_route, p_RouteSectionType);
                ...
                if (RouteVisCommonVars_DEPRECATED.Instance.CalcTRK_ETOLLCAT > 1 && p_lastETLCODE != edge.EDG_ETLCODE)
                {
                    //Az 
                    detail.Toll += edge.Tolls[RouteVisCommonVars_DEPRECATED.Instance.CalcTRK_ETOLLCAT] * p_TollMultiplier;
